...
static unsigned char tmpNum = 0;
static unsigned char tmpNumReady = 0;
static unsigned char numStr[4];


static unsigned char sendNum(unsigned char n, unsigned char nextSub, unsigned char terminator) {
    if (!tmpNumReady) {
        unsigned char i = 0;
        if (n >= 100) numStr[i++] = '0' + n / 100;
        if (n >= 10) numStr[i++] = '0' + (n / 10) % 10;
        numStr[i++] = '0' + n % 10;
        numStr[i] = '\0';
        tmpNumReady = 1;
    }
    if (numStr[si]) { SIO_PutChar(numStr[si++]); return 1; }
    si = 0;
    if (terminator == '$') { SIO_PutChar('$'); }
    else { SIO_PutChar('\r'); SIO_PutChar('\n'); }
    tmpNum = 0; tmpNumReady = 0;
    subestado = nextSub;
    return 0;
}
...
void SIO_PutChar(unsigned char c) { while (!SIO_TXAvail()); TXREG = c; }

unsigned char nextHead(unsigned char h) { return (h + 1) & (2 - 1); }
...
void motorSIO(void) {
    ...
    if (estado == 0) {
        ...
        msgActual = &cola[tail];
        tail = nextHead(tail);
        si = subestado = tmpNum = tmpNumReady = 0;
        estado = 1;
        return;
    }
    ...
    if (tipo >= SEND_SELECT) {
        if (msgSimple[tipo][si]) { SIO_PutChar(msgSimple[tipo][si++]); return; }
    ...
    if (tipo == SEND_DATA_ANIMAL) {
        switch (subestado) {
            ...
            case 1: {
                ...
                if (sendStr(animalStr[(t <= 3) ? t : 4], 2)) return;
                break;
            }
            case 2:
