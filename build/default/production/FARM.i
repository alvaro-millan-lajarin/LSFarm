...
static unsigned char *farmNameSrc = ((void*)0);
...
static unsigned char nuevoAnimal = 0;
static unsigned char tipusUltimoAnimal = 0;
static unsigned char numPorTipo[4] = {0, 0, 0, 0};

static unsigned char nuevoAnimalNum = 0;
unsigned char getNumUltimoAnimal(void) { return nuevoAnimalNum; }
unsigned char getFarmNameReady(void) { return !startSetFarmName; }
void resetNuevoAnimal(void) { nuevoAnimal = 0; }
unsigned char* getFarmName() { return nameFarm; }
unsigned char hayNuevoAnimal(void) { return nuevoAnimal; }
unsigned char getTipusUltimoAnimal(void) { return tipusUltimoAnimal; }
...
void DO_SetCowTime(unsigned int time) { animalTimes[COW] = time; }
void DO_SetHorseTime(unsigned int time) { animalTimes[HORSE] = time; }
void DO_SetPigTime(unsigned int time) { animalTimes[PIG] = time; }
void DO_SetHenTime(unsigned int time) { animalTimes[CHICKEN] = time; }

void crearAnimal(unsigned char tipus) {
    if (numAnimals >= 24) return;
    EEPROM_WriteAnimal(numAnimals, tipus);
    numPorTipo[tipus]++;
    tipusUltimoAnimal = tipus;
    nuevoAnimalNum = numPorTipo[tipus];
    nuevoAnimal = 1;
    numAnimals++;
}
...
void motorFarm() {
    ...
    switch (estat) {
        ...
        case 0: {
            ...
            if (gameStart && !EEPROM_IsBusy() && numPorTipo[t] < 15 &&
