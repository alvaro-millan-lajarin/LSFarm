...
typedef struct {
    send_t tipo;
    unsigned char animalTipus;
    unsigned char animalNum;
    unsigned char animalEstat;
...
unsigned char EEPROM_GetEstat(AnimalEEPROM a, unsigned char horaAct,
                              unsigned char minAct, unsigned char segAct);
...
static unsigned char wait5sec = 1;



cmd_t COMMAND_Parse() {
    if (!getCmdReady()) return CMD_NONE;
    unsigned char* c = getCommand();

    switch (c[0]) {
        case 'I': return CMD_INIT;
        case 'P': return CMD_GET_PRODUCTS;
        case 'A': return CMD_GET_ANIMALS;
        case 'R': return CMD_RESET;
        case 'C': return CMD_CONSUME;
        case 'E': return CMD_SLEEP;
        case 'L': return CMD_START_REBELLION;
        case 'S': return CMD_STOP_REBELLION;
        default: return CMD_UNKNOWN;
    }
}

static unsigned char findAnimalIndex(unsigned char *data) {
    unsigned char tipus;
    if (data[0]=='V') tipus = 0;
    else if (data[0]=='P' && data[1]=='O') tipus = 1;
    else if (data[0]=='C') tipus = 2;
    else if (data[0]=='G') tipus = 3;
    else return 0xFF;


    unsigned char k = 0;
    while (data[k] != '$' && data[k] != '\0') k++;
    unsigned char num = data[k+1] - '0';

    unsigned char total = getNumTotalAnimals();
    unsigned char count = 0;
    unsigned char j;
    for (j = 0; j < total; j++) {
        AnimalEEPROM an = EEPROM_ReadAnimal(j);
        if (an.tipus == tipus) {
...
cmd_t handle_GetAnimals(void) {
    if (i == 0) {
        totalAnimals = getNumTotalAnimals();
            ...
            unsigned char estat = EEPROM_GetEstat(an, DT_GetHora(), DT_GetMin(), DT_GetSeg());
            unsigned char numero = 0;
            unsigned char k;
            for (k = 0; k < i; k++) {
                if (EEPROM_ReadAnimal(k).tipus == an.tipus) numero++;
            }
...
void motorParseCommand(){
    ...
    switch (cmd) {
        ...
        case CMD_SLEEP:
            ...
            if((unsigned int)TI_GetTics(16) > (unsigned int)24040){
                if(getSleep()){
                    unsigned char idx = findAnimalIndex(getData());
                    EEPROM_WriteAnimalSleep(idx);
