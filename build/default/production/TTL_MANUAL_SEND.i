...
# 13 "TTL_MANUAL_SEND.c" 2
# 27 "TTL_MANUAL_SEND.c"
unsigned char estado = 0;
unsigned char dato;
unsigned char ok_transmit = 1;

static const unsigned char *cola[8];
...
int HanPasado208usTX(){

    return((unsigned int)TI_GetTics(7) >= 2);
}
void SIO_Manual_SetFrase(unsigned char *msg) {
    unsigned char next = (head + 1) & 7;
...
void motorSIOSend(){
    ...
    switch(estado){
        ...
        case 1:
            if(HanPasado208usTX()){
                PORTCbits.RC0 = (dato >> bit_pos) & 1;
                TI_ResetTics(7);
                bit_pos++;
                if(bit_pos == 8) estado = 2;
            }
        break;
