...
# 10 "PARSE_COMMAND.c" 2
# 1 "./PARSE_COMMAND.h" 1
# 11 "PARSE_COMMAND.c" 2
# 1 "./COMMANDS.h" 1
...
cmd_t COMMAND_Parse(void){

    if(!cmd_ready) return CMD_NONE;
    cmd_ready = 0;

    if(strcmp(COMMAND, "CMD_INITIALIZE") == 0){
        return CMD_INIT;
    }
    else if(strcmp(COMMAND, "GET_ANIMALS") == 0){
        return CMD_GET_ANIMALS;
    }
    else if(strcmp(COMMAND, "GET_PRODUCTS") == 0){
        return CMD_GET_PRODUCTS;
    }
    else if(strncmp(COMMAND, "CONSUME", 7) == 0){
        return CMD_CONSUME;
    }
    else if(strcmp(COMMAND, "RESET") == 0){
        return CMD_RESET;
    }
    else if(strcmp(COMMAND, "START_REBELLION") == 0){
        return CMD_START_REBELLION;
    }
    else if(strcmp(COMMAND, "STOP_REBELLION") == 0){
        return CMD_STOP_REBELLION;
    }
...
void motorParseComnnad(){
    ...
    switch (estat){
        case 0:
            ...
            break;
        case 1:
            static cmd_t cmd = COMMAND_Parse();
            estat = 2;
