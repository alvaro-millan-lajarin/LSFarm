...
unsigned char writeAddr[8];
unsigned char writeVal[8];
...
unsigned char EEPROM_GetEstat(AnimalEEPROM a, unsigned char horaAct,
    ...
    int diffMin = (int)(horaAct*60 + minAct) - (int)(a.hora*60 + a.min);
    if (diffMin < 0) diffMin += 1440;

    if (diffMin > 2) return 1;
    if (diffMin == 2) {
        return (segAct >= a.seg) ? 1 : 0;
    }
    return 0;
}
