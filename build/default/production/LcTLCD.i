...
void DO_SetCowTime(unsigned int time);
void DO_SetHorseTime(unsigned int time);
void DO_SetPigTime(unsigned int time);
void DO_SetHenTime(unsigned int time);
...
unsigned char hayNuevoAnimal(void);
unsigned char getTipusUltimoAnimal(void);

void resetNuevoAnimal(void);
unsigned char getFarmNameReady(void);

unsigned char getNumUltimoAnimal(void);
# 6 "LcTLCD.c" 2
# 1 "./PRODUCTS.h" 1
...
unsigned char hayNuevoProducto(void);
unsigned char getTipusUltimoProducto(void);
unsigned char getNumUltimoProducto(void);
void resetNuevoProducto(void);
# 7 "LcTLCD.c" 2
# 1 "./game.h" 1
...
static const char * const HEADS[2] = { s0, s1 };
static const char * const NOMS[2][4] = {
    { s2, s4, s5, s3 },
    { s6, s8, s9, s7 }
};
...
static void cantaByte(unsigned char val, unsigned char rs) {
    (TRISDbits.TRISD4 = TRISDbits.TRISD5 = TRISDbits.TRISD6 = TRISDbits.TRISD7 = 0);
    if (rs) (LATDbits.LATD1 = 1); else (LATDbits.LATD1 = 0);
    (LATDbits.LATD2 = 0);
    (LATDbits.LATD3 = 1);
    cantaNibble(val >> 4);
    (LATDbits.LATD3 = 0);
    ...
    (LATDbits.LATD3 = 0);
    (TRISDbits.TRISD4 = TRISDbits.TRISD5 = TRISDbits.TRISD6 = TRISDbits.TRISD7 = 1);
    esperaMs(2);
}
...
void LcInit(char rows, char columns) {
    (TRISDbits.TRISD3 = TRISDbits.TRISD2 = TRISDbits.TRISD1 = 0);
    esperaMs(192);
    (TRISDbits.TRISD4 = TRISDbits.TRISD5 = TRISDbits.TRISD6 = TRISDbits.TRISD7 = 0); (LATDbits.LATD1 = 0); (LATDbits.LATD2 = 0);
    (LATDbits.LATD3 = 1); cantaNibble(0x03); (LATDbits.LATD3 = 0); esperaMs(24);
    (LATDbits.LATD3 = 1); cantaNibble(0x03); (LATDbits.LATD3 = 0); esperaMs(5);
    (LATDbits.LATD3 = 1); cantaNibble(0x03); (LATDbits.LATD3 = 0); esperaMs(5);
    (LATDbits.LATD3 = 1); cantaNibble(0x02); (LATDbits.LATD3 = 0); esperaMs(5);
    cantaByte((0x28), 0);
    cantaByte((0x08), 0);
    cantaByte((0x01), 0);
    esperaMs(8);
...
void LcGotoXY(char Column, char Row) {
    cantaByte((0x80 | (Column + (Row ? 0x40 : 0))), 0);
}
...
void MotorLCD(void) {
    ...
    if (hayNuevoAnimal()) {
        Notificacio n;
        n.tipo = 'A';
        n.nombre = getTipusUltimoAnimal();
        n.num = getNumUltimoAnimal();
        resetNuevoAnimal();
        addMsgToQueue(n);
    } else if (hayNuevoProducto()) {
        Notificacio n;
        n.tipo = 'P';
        n.nombre = getTipusUltimoProducto();
        n.num = getNumUltimoProducto();
        resetNuevoProducto();
        addMsgToQueue(n);
    }
    ...
    switch (state) {
        ...
        case 5: {
            ...
            unsigned char d = DT_GetDia();
            unsigned char m = DT_GetMes();
            LcGotoXY(0, 1);

            num_buf[0] = '0' + (d >= 20 ? 2 : d >= 10 ? 1 : 0);
            num_buf[1] = '0' + (d >= 20 ? d-20 : d >= 10 ? d-10 : d);
            num_buf[2] = '/';
            num_buf[3] = '0' + (m >= 10 ? 1 : 0);
            num_buf[4] = '0' + (m >= 10 ? m-10 : m);
            num_buf[5] = '/';
            num_buf[6] = '2';
            num_buf[7] = '0';
            num_buf[8] = '2';
        ...
        case 21:
            LcGotoXY(0, 1);
            str = NOMS[current.tipo != 'A'][current.nombre];
            index = 0;
            next_state = 22;
            state = 10;
            break;
        ...
        case 22: {
            ...
            num_buf[i++] = ':';
            num_buf[i++] = ' ';
            if (current.num >= 100) num_buf[i++] = '0' + current.num / 100;
            if (current.num >= 10) num_buf[i++] = '0' + (current.num / 10) % 10;
            num_buf[i++] = '0' + current.num % 10;
            num_buf[i] = '\0';
            str = num_buf;
            index = 0;
            next_state = 23;
        ...
        case 24:
            if ((unsigned int)TI_GetTics(motorTimer) >= ((unsigned int)14423)) {
                idleShown = 0;
                state = 0;
            }
            break;
        ...
        case 10:
            if (str[index] == '\0') { index = 0; state = next_state;
            break;
            }
            LcPutChar(str[index++]);
            break;
